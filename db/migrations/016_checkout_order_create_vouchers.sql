-- checkout_order: unterstützt jetzt zusätzlich das Erstellen neuer Gutscheine
-- (Gutschein-Verkauf an der Kasse), als Teil derselben atomaren Transaktion.
-- Neuer Parameter -> alte Signatur zuerst explizit droppen (sonst entsteht eine
-- zweite, überladene Funktion statt eines Ersatzes).
drop function if exists checkout_order(uuid, uuid, uuid, numeric, text, numeric, numeric, jsonb, jsonb);
