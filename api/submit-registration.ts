...
export default async function handler(req: any, res: any) {
  ...
  const { customerId, profile, treatmentType, treatmentDetail, answers, healthNoticeText, consentAccepted } = req.body || {};
  ...
  try {
    const { data: existingCustomer, error: findError } = await admin.from('customers').select('id').eq('id', customerId).single();
    ...
    const { error: updateError } = await admin.from('customers').update(customerPatch).eq('id', customerId);
    if (updateError) {
